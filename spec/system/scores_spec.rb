RSpec.describe 'Scores', type: :system do
  ...
  describe 'User' do
    ...
    it 'shows link to top when a user has scores' do
      user_score.save
      sign_in user
      visit welcome_path
      expect(page).to have_content 'ドンナトコは登録済みです'
      expect(page).to have_content 'トップページへ'
